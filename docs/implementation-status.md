...
- 网页资料列表展示自动分类、自动标签、网页摘要、站点和封面；资料保存后先显示处理状态。
- 支持按自动分类筛选，网页资料重复规范网址自动跳过。
- Web 主资料列表支持按自动分类、资源形态、标签、收藏夹、星标、条目状态和创建日期组合筛选，并可按更新时间/创建时间/标题排序；阅读进度在资料详情中查看，巡检状态在网页巡检中心中查看和处理。
...
- 网页资料列表与详情展示自动分类、资源形态、阅读进度、收藏夹、摘要、站点、健康状态和原始网址，支持打开、复制和分享；这些字段在 Android 端只读。
...
| Android 资料只读字段 | 服务端同步资源形态/阅读进度/收藏夹 → 资料详情 | 显示“课程 / 阅读中 / P0·课程”，无编辑表单 |
...
- Web 暂未提供离线编辑。
- Web 已支持关键词、类型、自动分类、标签、收藏夹、资源形态、星标、条目状态、创建日期和排序组合；阅读进度保留在资料详情中，巡检状态保留在网页巡检中心中；Android MVP 仍以统一本地搜索和单列只读浏览为主。
